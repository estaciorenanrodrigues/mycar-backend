...
const atualizarVeiculo = (req, res) => {
  try {
    ...
    // Verificar se placa, chassi ou renavam já existem em outro veículo
    const placaExistente = veiculos.find(v => v.placa === placa && v.id !== Number.parseInt(id, 10));
    const chassiExistente = veiculos.find(v => v.chassi === chassi && v.id !== Number.parseInt(id, 10));
    const renavamExistente = veiculos.find(v => v.renavam === renavam && v.id !== Number.parseInt(id, 10));
